"""Adaptive Phase v5 — Hard-Boiled Physical Law & Absolute Safety Mask.

Strictly enforces:
  1. Net Damage = 45 - 23 = 22. 5-shot death limit aligned perfectly.
  2. 5% Undead / Perishing stategy: maximum exchange on lowest HP,
     stay in friendly fire-zones.
  3. Range-31 Full-field coverage threat estimation (Move 22 + Range 9).
  4. Precise Kiting Edge Selection (Distance < 9 but closest to 9
     when all else fails).
"""
...
class UltimateTacticalState:
    ...
    def get_filtered_actions(self, legal_moves: List[Point]) -> List[Point]:
        """Hard-rule decision filter: trade / kite / safest."""
        if not legal_moves:
...
def get_adaptive_phase_v5_init_strategy() -> Callable[..., List[PieceArg]]:
...
def get_adaptive_phase_v5_action_strategy() -> Callable[..., ActionSet]:
    """Safety-hardened decision core with hard-rule filters."""

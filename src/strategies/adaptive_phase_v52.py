"""Adaptive Phase v52 — Emergency break-contact when surrounded.

Fixes over v51:
  1. Emergency retreat: when d <= 3 from nearest enemy AND outnumbered
     (danger >= 2), allow retreat beyond d=9 without penalty.
  2. Soft kiting decay: d > 9 uses slow decay (9 - (d-9)*0.3) instead
     of hard -float(d) penalty, so retreat remains viable.
"""
...
# ==============================================================================
#            2. Safety state with emergency break-contact
# ==============================================================================
...
class TacticalState:
    ...
    def _nearest_enemy_dist(self, pos: Point) -> int:
        if not self.enemies:
            return 999
        return min(manhattan(pos, e.position) for e in self.enemies)
...
def get_adaptive_phase_v52_init_strategy() -> Callable[..., List[PieceArg]]:
...
def get_adaptive_phase_v52_action_strategy() -> Callable[..., ActionSet]:
